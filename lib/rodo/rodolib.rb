...
class TodoDay
  ...
  class Wrap
    ...
    def eql?(other_object)
      other_object == self || (other_object != nil && other_object.delegate[:text] == @delegate[:text])
    end

    #require 'forwardable'
    #extend Forwardable
    #def_delegators :@delegate, :to_s, :inspect
...
#     #t.diff(s){ |action, position, element|
...
#     t.sdiff(s) { |action, (old_pos, old_element), (new_pos, new_element)|
...
#       case action
#       when '='
#         merge_structures(old_element.delegate[:children], new_element.delegate[:children])
#         index += 1
...
#         # insert new_element
#         target.insert(index, new_element.delegate)
#         index += 1
